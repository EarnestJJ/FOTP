...
library(stringr)
library(data.table)
setwd("fragment_detail_path")  ###The path to extract 10bp sequences information
sapply(c("FOTP","SOTP","single_base_freq"),dir.create)
...
for (i in c(1:length(heal_file))){    
    ...
    write.table(matrix_2mer_trans,file=paste0("./FOTP_2mer/",CN[i],".txt"))
    ...
    write.table(matrix_3mer_trans,file=paste0("./SOTP_3mer/",CN[i],".txt"))    
}
